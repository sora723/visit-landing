...
export function ReservationToast() {
  ...
  const [toast, setToast] = useState<ToastState>({ isDuplicate: false });

  useEffect(() => {
    const show = (event: Event) => {
      const detail = (event as CustomEvent<{ isDuplicate?: boolean }>).detail;
      setToast({ isDuplicate: detail?.isDuplicate === true });
      setVisible(true);
